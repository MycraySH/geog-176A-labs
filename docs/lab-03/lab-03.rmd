...
totalpop = eqdc_uscities %>%
  select(population) %>%
  summarise(sum(population))

border_zone = eqdc_uscities  %>%
  filter(as.numeric(distance_border <= 160))%>%
  summarise(number_cities = n(), sumpop = sum(population), percent = 100 * (sum_pop/totalpop[1])) %>%
  st_drop_geometry() 
  knitr::kable(border_zone, caption = "Cities In the 100 Mile Zone of The US Boarder",
...
ggplot() +
  geom_sf(data = data) +
  geom_sf(data = eqdc_uscities+
            aes(col = as.numeric(distance_border)), size = 0.5) +
  gghighlight::gghighlight(as.numeric(cities_eqdc$dist_border) <= 160) +
  scale_color_gradient(low = "reddish purple", high = "red", name = "Distance (km)") +
  ...
  ggrepel::geom_label_repel(
    ...
    stat = "sf_coordinates",
    size = 2) +
   labs(title = "Cities In the 100 Mile Zone of The US Boarder",
       x = " ",
       y = " ") + 
  theme_dark() 
```
