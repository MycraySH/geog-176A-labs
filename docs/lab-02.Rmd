...
popcapita = capita %>% 
  ...
  mutate(cumul_cases_pcapita = (cases/newpop)) %>% 
  mutate(new_cases_pcapita = (Ncases2/newpop))%>% 
  filter(date == max(date))
...
cumul_capita = popcapita %>%
  slice_max(cumul_cases_pcapita, n = 5) %>%
  arrange(-cumul_cases_pcapita) %>% 
...
new_capita = popcapita %>%
    filter(date == max(date)) %>%
    slice_max(new_cases_pcapita, n = 5) %>%
    arrange(-new_cases_pcapita) %>%
