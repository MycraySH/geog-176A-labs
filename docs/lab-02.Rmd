...
#recent 14 days data
```{r, message = FALSE, warning = FALSE}
#covidpcap = right_join(popcapita, casedata, by = "fips")
#recent14 = covidpcap %>%
  #select(county = county.y, Ncases, newpop, date) %>%
  #group_by(county, newpop) %>%
  #summarise(Ncases14 = sum(Ncases, na.rm = TRUE)) %>%
  #mutate(new_milion = (newpop/(newpop/100000))) %>% 
  #filter(date >= max(date) - 13)-> 14cases
```
#Safe Counties
```{r, message = FALSE, warning = FALSE}
safecounties = popcapita %>%
  group_by(county, newpop) %>%
...
```{r, message = FALSE, warning = FALSE}
covid2  %>% 
  ggplot(aes(x = as.Date(date), y=Ncases)) +
  geom_col(aes(y = NCases, col = state)) +
