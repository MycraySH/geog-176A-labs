...
#3.4
```{r,message=FALSE, warning=FALSE}
plot_in_polygon = function(data, title)
  {ggplot() +
    geom_sf(data = data, aes(fill = log(n)), alpha = 1, size = .2, col = NA) +
    scale_fill_viridis_c()+
    theme(legend.position = 'none',
          plot.title = element_text(face = "bold", color = "red", size = 20,hjust = .5)) +
    theme_dark() +
    labs(title = title,
...
#3.5
```{r,message=FALSE, warning=FALSE}
plot_in_polygon(oripol, "Number of Dams In Countries")
plot_in_polygon(vorpol, "Number of Dams by Voroni Pattern")
plot_in_polygon(trianpol, "Number of Dams by Triangle Pattern")
plot_in_polygon(gridpol, "Number of Dams by Grid Pattern")
plot_in_polygon(hexpol, "Number of Dams by Hexagon Pattern")
```
